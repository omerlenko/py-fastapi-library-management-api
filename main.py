...
def create_author(data: schemas.AuthorCreate, db: DbDep):
    ...
    if author:
        raise HTTPException(status_code=400, detail="An author with this name already exists")
...
def create_book(data: schemas.BookCreate, db: DbDep):
    ...
    if book is None:
        raise HTTPException(status_code=400, detail="The chosen author for this book does not exist")
    return book